...
df = pd.read_csv("./Dataset-Grid view.csv")  # df means dataframe
df = df.fillna('')
...
for idx,row in df.iterrows():
    ...
    dataset_content["last_update_date"] = mdy_to_ymd(row['最後更新日期'])
    dataset_content["datasource"] = row['資料來源']
    dataset_content["region"] = row['地區']
    dataset_content["updatefreq"] = row['更新頻率']
    dataset_content["start_date"] = mdy_to_ymd(row['數據開始日期'])
    dataset_content["end_date"] = mdy_to_ymd(row['數據結束日期'])
